...
function parse_tweet($tweet) {
  ...
  ret .= "<a class='time' href='".path_join(BASE_URL, "tweet/show", $tweet->id_str)."'>".format_time(strtotime($tweet->created_at))."</a>";
...
function echo_tweets() {
  ...
  for ($content['tweets'] as $tweet) {
    echo "<li".is_user_mention_in_tweet($access_token['screen_name'])?" class='mentioned'":"".">";
